...
TITLE REQUIREMENT:
- For every selected category, title is REQUIRED and must never be null or empty.
- Generate the marketplace title from the source message itself.
- The title MUST always be written in English.
- If the source message is not English, translate its meaning into natural English.
...
For the selected category, use the exact field names above. Every selected-category field is required by the output contract; use null or [] when the value is unknown, except title which must always contain a non-empty English title.
"""
...
def _fallback_title(source_text):
    """Return a non-empty source-derived title if the provider omitted title.

    This is a safety net only. The normal path remains AI-generated English titles.
    It prevents an otherwise valid listing from being persisted with a meaningless
    empty title when Groq returns JSON without the required field.
    """
