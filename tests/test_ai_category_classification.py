...
def test_category_classification_service_batches_and_marks_results(monkeypatch, tmp_path):
    ...
    assert stats == {"found": 2, "processed": 2, "failed": 0, "skipped": 0, "stopped": False, "provider_configuration_error": False, "disabled": False}
...
def test_category_classification_marks_a_permanent_provider_failure(monkeypatch, tmp_path):
    database, config = _load_database(monkeypatch, tmp_path)
    from ai.classification_service import CategoryClassificationService
    ...
    class ForbiddenClassifier:
        def classify_batch(self, messages):
            raise AIExtractionError("Groq classification failed: status=403", status=403, reason="permissions_error", stop_queue=True)
    ...
    stats = CategoryClassificationService(repository=repo, classifier=ForbiddenClassifier(), batch_size=config.AI_CLASSIFICATION_BATCH_SIZE).process_pending()

    assert stats["failed"] == 1
    assert stats["stopped"] is True
    assert stats["provider_configuration_error"] is True
